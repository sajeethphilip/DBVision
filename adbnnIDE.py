...
class AdaptiveDBNNGUI:
    ...
    def _execute_adaptive_via_command_line(self):
        ...
        try:
            ...
            # ADD VISUALIZATION FLAG BASED ON GUI SETTING
            if self.enable_visualization_var.get():
                cmd.append("--visualize")
                self.log_output("🎨 Visualization ENABLED for this run")
            else:
                self.log_output("🔇 Visualization DISABLED for this run")
    ...
    def update_visualization_status(self):
        """Update visualization status display"""
        try:
            status = self.check_visualization_status()
            self.log_output(f"📊 Visualization Status: {status}")
        except Exception as e:
            self.log_output(f"⚠️ Could not check visualization status: {e}")
...
class AdaptiveDBNN:
    ...
    def _execute_command_line_adaptive(self):
        ...
        try:
            ...
            return self._execute_command_line_adaptive()
...
class FunctionalIntegratedAdaptiveDBNN:
   ...
   ╔═════════════════🧠      DBNN CLASSIFIER ═════════════════════════╗
    ...
    def open_specific_visualization(self, viz_type):
        """Open specific visualization subdirectory"""
        try:
            if not hasattr(self, 'adaptive_model') or not self.adaptive_model:
                messagebox.showwarning("Warning", "No model available. Please run adaptive learning first.")
                return

            # Determine the base visualization directory
            if hasattr(self.adaptive_model, 'comprehensive_visualizer'):
                base_dir = self.adaptive_model.comprehensive_visualizer.output_dir
            elif hasattr(self.adaptive_model, 'visualizer'):
                base_dir = self.adaptive_model.visualizer.output_dir
            else:
                self.log_output("❌ No visualizer found. Generate visualizations first.")
                return

            # Map visualization types to subdirectories
            viz_dirs = {
                "performance": "performance",
                "networks": "networks",
                "samples": "sample_evolution",
                "distributions": "distributions",
                "comparisons": "comparisons",
                "interactive": "interactive"
            }

            if viz_type in viz_dirs:
                viz_dir = base_dir / viz_dirs[viz_type]
                if viz_dir.exists():
                    self._open_directory_in_explorer(viz_dir)
                    self.log_output(f"📂 Opened {viz_type} visualization folder")
                else:
                    self.log_output(f"❌ {viz_type} visualization folder not found. Generate visualizations first.")
            else:
                self.log_output(f"❌ Unknown visualization type: {viz_type}")

        except Exception as e:
            self.log_output(f"❌ Error opening {viz_type} visualizations: {e}")
    ...
    def _open_directory_in_explorer(self, directory):
        ...
        try:
            ...
            self.log_output(f"❌ Error opening directory: {e}")

    def check_visualization_status(self):
        """Check and report visualization status"""
        try:
            if not hasattr(self, 'adaptive_model') or not self.adaptive_model:
            ...
            if viz_dir.exists():
                ...
                status = f"Visualizations: {len(html_files)} HTML, {len(png_files)} PNG, {len(gif_files)} GIF"

                # Check for main dashboard
                dashboard_files = list(viz_dir.rglob("*dashboard*.html"))
                if dashboard_files:
                    status += f", Dashboard: ✅"
                else:
                    status += f", Dashboard: ❌"
...
"""
    ...
    def setup_visualization_controls(self):
        """Setup visualization controls in the training tab"""
        # Add visualization controls to training tab
        viz_frame = ttk.LabelFrame(self.training_tab, text="🎨 Visualization Controls", padding="10")
        viz_frame.pack(fill=tk.X, padx=10, pady=10)

        # Visualization toggle
        viz_check_frame = ttk.Frame(viz_frame)
        viz_check_frame.pack(fill=tk.X, pady=5)

        ttk.Checkbutton(viz_check_frame, text="Enable Visualizations During Training",
                       variable=self.enable_visualization_var).pack(side=tk.LEFT, padx=5)

        # Visualization action buttons
        viz_button_frame = ttk.Frame(viz_frame)
        viz_button_frame.pack(fill=tk.X, pady=5)

        ttk.Button(viz_button_frame, text="📊 Generate Final Visualizations",
                  command=self.generate_final_visualizations).pack(side=tk.LEFT, padx=2)
        ttk.Button(viz_button_frame, text="📁 Open Visualization Folder",
                  command=self.open_visualization_location).pack(side=tk.LEFT, padx=2)
        ttk.Button(viz_button_frame, text="🎬 Show Animations",
                  command=self.show_animations).pack(side=tk.LEFT, padx=2)
        ttk.Button(viz_button_frame, text="🌐 Show Interactive 3D",
                  command=self.show_interactive_3d).pack(side=tk.LEFT, padx=2)

        # Quick access to specific visualizations
        quick_viz_frame = ttk.Frame(viz_frame)
        quick_viz_frame.pack(fill=tk.X, pady=5)

        ttk.Label(quick_viz_frame, text="Quick Access:").pack(side=tk.LEFT, padx=5)
        ttk.Button(quick_viz_frame, text="Performance Plots",
                  command=lambda: self.open_specific_visualization("performance")).pack(side=tk.LEFT, padx=2)
        ttk.Button(quick_viz_frame, text="3D Networks",
                  command=lambda: self.open_specific_visualization("networks")).pack(side=tk.LEFT, padx=2)
        ttk.Button(quick_viz_frame, text="Sample Evolution",
                  command=lambda: self.open_specific_visualization("samples")).pack(side=tk.LEFT, padx=2)
